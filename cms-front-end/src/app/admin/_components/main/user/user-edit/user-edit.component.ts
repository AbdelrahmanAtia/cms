...
import { AuthorityService } from 'src/app/admin/_services/authority.service';
import { Authority } from 'src/app/admin/_models/Authority';
...
export class UserEditComponent implements OnInit {
  ...
  private addNewUser(user: User): void {
    this.userService.addNewUser(user).subscribe(
      (response: User) => this.router.navigate(['admin/main', 'users', ' ', 'All','1'])
      , (error) => console.log(error)
    );
  }
  ...
  private updateUser(user: User): void {
    ...
    this.userService.updateUser(user).subscribe(
      (response: User) => {
        ...
        this.router.navigate(['admin/main', 'users', ' ','All', '1']);
      }
      , (error) => console.log(error)
    );
  }
