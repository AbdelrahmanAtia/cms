...
export class UserListComponent implements OnInit {
  ...
  addNewUser(): void {
    this.router.navigate(['admin/main', 'users', 'new']);
  }

  editUser(userId: number): void {
    this.router.navigate(['admin/main', 'users', userId, 'edit']);
  }
      ...
      (response: Response) => {
        if (response.status) {
          if (this.pageNumber > 1 && this.totalPages == this.pageNumber && this.users.length == 1) {
            ...
            this.router.navigate(['admin/main', 'users', (this.searchTerm.length > 0) ? this.searchTerm : " ", this.userStatus, this.pageNumber - 1]);
          } else {
            //stay in the same page
            this.router.navigate(['admin/main', 'users', (this.searchTerm.length > 0) ? this.searchTerm : " ", this.userStatus, this.pageNumber, new Date().getTime()]);
          }
        } else {
  ...
  onSearchChange(event) {
    if (event.keyCode == 13) {      // enter key pressed
      this.router.navigate(['admin/main', 'users', (this.searchTerm.length > 0) ? this.searchTerm : " ", 'All', '1']);
    }
  }

  onPageChange(i: number): void {
    let pageNumber: number = this.pageNumber + i;
    this.router.navigate(['admin/main', 'users', (this.searchTerm.length > 0) ? this.searchTerm : " ", this.userStatus, pageNumber]);
  }

  onStatusChange(status: string): void {
    this.router.navigate(['admin/main', 'users', ' ', status, '1']);
  }
